  ...
  return (
    <section className="placeholder-fill-panel">
      <div className="placeholder-fill-list">
        {cards.map((card) => {
          const expanded = collapsedFields[card.id] !== true;
          ...
          return (
            <article className="placeholder-variable-card placeholder-fill-card complex-fill-work-card" key={card.id}>
              ...
              {card.outputRequirement ? (
                <p className="complex-fill-output-rule" title={card.outputRequirement}>{card.outputRequirement}</p>
              ) : null}
