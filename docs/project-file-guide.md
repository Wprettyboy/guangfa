...
| `scripts/check-zvec-sparse.mjs` | ZVec Sparse 独立稳定性门禁。验证批量 CRUD、三路查询、结构过滤、优化和关闭重开一致性。 |
...
| `scripts/evaluate-knowledge-retrieval.mjs` | V4 检索评测。支持人工标注或当前库精确证据探针，计算 Recall@20/Top-5，并可交替压测 MinerU VLM 与 BGE GPU 推理。 |
...
| `scripts/rebuild-knowledge-index-v4.mjs` | V4 知识索引重建。读取 SQLite 存活子块，构建并验证不可变 Dense/Sparse/FTS generation，再原子切换活动 manifest。 |
| `scripts/retrieval_model_server.py` | AMD Retrieval 模型服务。一次 BGE-M3 推理返回 Dense/Sparse，确定性裁剪 Sparse，并提供 Top-20 微批精排、GPU 队列截止时间和显存健康指标。 |
...
### 8.5 `server/retrieval/` 检索模型适配

| 文件 | 功能说明 |
| --- | --- |
| `server/retrieval/circuit-breaker.js` | 独立轻量熔断器。连续失败后开启 30 秒，只允许一个半开探针，成功恢复、失败重开。 |
| `server/retrieval/model-client.js` | Retrieval HTTP 客户端。分别执行 Query/Index Encode 与 Rerank 的截止时间、协议校验、固定降级码和入库单次退避重试。 |

### 8.6 `server/solution-writing/` 方案生成
...
| `src/features/knowledge/KnowledgeSearchFilters.jsx` | 管理页结构化检索筛选。提供资料、页码、正文/表格、星号项和清除条件，并只发送显式过滤值。 |
...
| `src/pages/KnowledgeBaseManagement.jsx` | 知识库管理。管理项目/全局库、资料上传删除、结构筛选、检索诊断、降级提示、命中高亮和认证原文预览；viewer 只显示读取能力。 |
...
| `src/services/knowledgeBase.js` | 知识库 API 客户端。封装库/资料查询、创建、删除、文本筛选检索及表格/图片检索；上传自动携带稳定 `Idempotency-Key`。 |
