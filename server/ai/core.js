...
export async function createFormatOutlinePlan(payload) {
...
export function normalizeFillMode(field = {}) {
...
export function getFillModeLabel(mode) {
  return {
    short: "短文本填空",
    paragraph: "长文本填空",
    date: "日期填空",
...
export function getFillModePromptRule(mode) {
...
export function getFillOutputJsonPrompt(mode) {
...
export function createDefaultPackageOrSegmentResult(evidence) {
  return {
    value: "1",
    status: "待确认",
    confidence: 80,
...
export function extractChoiceReplacementCandidate(field = {}, knowledgeSnippets = [], materialSnippets = []) {
...
export function getChoiceReplacementThemeTerms(field = {}) {
  const context = [field.name, field.sourceText, field.templateContext, field.answerFormat, field.question].filter(Boolean).join(" ");
...
export function createChoiceReplacementFallbackResult(candidate) {
  return {
...
export function sanitizeChoiceFillResult(field, parsed, value, source, evidence) {
...
export function sanitizeAmountChoiceFillResult(parsed, amountValue, choiceValue, source, evidence) {
  const status = String(parsed?.status || "").trim();
...
export function createMissingChoiceResult(source, evidence) {
  return {
    value: "",
    status: "需补充资料",
    confidence: 0,
...
export function createNoRequirementChoiceResult(field, sourceBundle) {
