...
- `requestOnlyOfficeDeleteComplexFillAnchor(anchor)`：发送 `delete-complex-fill-anchor`，按 `GF_CF_` 书签精确选中范围；清除该选区高亮和删除书签是同一按钮触发的两个独立动作，不删除书签范围内的文档文字。
...
- `scripts/onlyoffice-outline-probe.js`
  ...
  - `deleteComplexFillAnchor(payload)`：先按 `GF_CF_` 书签精确选中范围；随后分别调用 `clearTextHighlightFromCurrentSelection()` 清除当前选区高亮、书签管理器的 `RemoveBookmark` / `asc_RemoveBookmark` 删除书签，回传 `bookmarkDeleted` 和 `highlight`，保留原文内容。
