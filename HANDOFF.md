...
- AMD Compose 新增三个可写 ROCm 缓存 volume：`guangfa-mineru-kernel-cache` -> `/root/.cache/comgr`、`guangfa-mineru-miopen-cache` -> `/root/.cache/miopen`、`guangfa-mineru-miopen-userdb` -> `/root/.config/miopen`。此前每次重建容器都要重新编译 ROCm 内核并重跑 MIOpen 卷积调优，缓存全空时首次解析实测约 210 秒，命中后约 10 秒；find-db（`/root/.config/miopen`）是三者中决定性的一个。2026-07-26 主机意外重启后复测：Docker 自动恢复容器，首次解析 33.9 秒、稳定态 9.3 秒（8 页 `测试.pdf`，与 Retrieval 共存），产物仍是 8 页、29 个块、6 个表格、1 张图片且 29/29 带 bbox，缓存卷内容（comgr 262MB、find-db `gfx1151_20.HIP...ufdb.txt` 2.1MB）跨重启存活。上文 139/158 秒是加缓存卷之前的数字，不要与之直接对照。`x-mineru-amd-common` 锚点与 `mineru-api` 服务各声明一次 `volumes`，服务级会整体覆盖锚点，新增挂载必须两处同步。
...
- AI 生图在服务端按“用户明确图型优先、当前标题图型次之”判定：流程图统一生成 PlantUML 活动图；功能组成图、功能结构图、功能分解图和明确 WBS 要求统一生成 `@startwbs` / `@endwbs` 结构图；其他要求选择一个主图型。
- 默认一张图只允许一个完整图块和一种图型语义，不使用 `newpage`，不拼接多个 start/end，也不使用 `!include`、`!includeurl` 或 `!import` 引入外部资源。用户明确要求组合展示时仍必须在一个主图型内用分区、分组或注释组织，不能混用不兼容的 PlantUML 图型语法。
- 活动图必须包含 `start`、至少一个 `:活动;` 和 `stop/end`，并拒绝 component、rectangle、node、package、database、cloud、queue、participant、class 等结构图节点。WBS 必须恰好一个 `*` 根节点、至少一个下级节点，并按 `**`、`***` 逐级展开，不能跨级。
- PlantUML 源码统一强制 `SimHei` 和 `defaultFontSize 20`；模型返回错误图型、组合图、无效层级或外部 include 时在渲染前失败并进入既有修复重试。多行用户生图要求保持换行，便于直接粘贴 WBS 结构示例。
- AI 生图弹窗每次打开都会重新读取当前 OnlyOffice 文档大纲，并在读取开始时清空旧标题；读取失败时不能继续使用上一文档上下文生成图片。图片渲染、DOCX 片段和 OnlyOffice 插入继续复用现有链路，无需修改注入脚本或缓存号。
- 已增加图型判定、活动图独占、旧 component/package 流程图拒绝、WBS 归一与层级、通用图型优先级、否定组合、单图块、外部 include 等回归。默认配置与 `ONLYOFFICE_SERVER_URL=http://127.0.0.1:8080` 下各 21 项测试、生产构建通过；当前 PlantUML `1.2026.6` 活动图和 WBS SVG 实际渲染均为 200。
...
- 已验证默认配置与 `ONLYOFFICE_SERVER_URL=http://127.0.0.1:8080` 下各 11 项回归测试、生产构建、相关 JS/Python 语法和 `git diff --check`；真实 Docker 补丁后 healthcheck 为 200，`ds:docservice` / `ds:converter` 为 `RUNNING`，`index.html` 加载 outline `gf=126`，Probe 本地、容器 `.js` 与 `.js.gz` 解压内容 SHA-256 一致。浏览器实际验证 DOCX 检索结果可从首页跳到物理第 43 页的 `4.6.1 业务流程` 标题，控制台无 warning/error。
...
- 当前有效缓存号：outline `gf=126`、placeholder `gf=31`、layout `gf=5`、RequireJS `urlArgs gf=25`、API `_dc=9.4.0-129-gf30`。
