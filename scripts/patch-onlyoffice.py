...
if "</body>" in html:
    html = html.replace("</body>", '<script src="guangfa-outline-probe.js?gf=91"></script>\n<script src="guangfa-placeholder-fields.js?gf=31"></script>\n<script src="guangfa-layout-format.js?gf=4"></script>\n</body>')
html = re.sub(r'urlArgs: "gf=\d+"', 'urlArgs: "gf=23"', html)
if 'urlArgs: "gf=23"' not in html:
    html = html.replace(
        "var require = {\n            waitSeconds: 30,",
        'var require = {\n            waitSeconds: 30,\n            urlArgs: "gf=23",',
    )
